  ...
  return (
    <div className="min-w-0 flex-1">
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        {featured && (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-accent/15 text-accent-bright border border-accent-dim/40">
            <IconPin className="w-3 h-3" />
            {t('updates_featured')}
          </span>
        )}
        <span
          className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border ${style.chip}`}
        >
          {t(KIND_LABEL_KEYS[entry.kind] ?? 'updates_tag')}
        </span>
        {entry.is_new && (
          <span className="text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md bg-accent/15 text-accent-bright border border-accent-dim/30">
            {t('updates_new')}
          </span>
        )}
        {entry.created_at > 0 && (
      ...
      {entry.command && (
        <div className="mt-3 flex items-center gap-2 rounded-lg bg-base border border-line px-3 py-2.5">
          ...
          <button
            ...
            className="focus-ring cursor-pointer shrink-0 p-1.5 rounded-md text-muted/60 hover:text-ink hover:bg-raised transition-colors"
          >
            {copied ? (
...
function CardMeta({ entry, featured }: { entry: UpdateEntry; featured?: boolean }) {
  const { t } = useTranslation('common')
  const style = KIND_STYLES[entry.kind] ?? KIND_STYLES.announcement
    ...
    <div className="flex flex-col items-end gap-2 shrink-0">
      ...
      {entry.link && /^https?:\/\//i.test(entry.link) && (
        <button
          ...
          className="focus-ring cursor-pointer flex items-center gap-1 px-2 py-1 rounded-md border border-line text-[10px] font-medium text-muted hover:text-ink hover:border-accent-dim transition-colors"
        >
...
function FeaturedCard({ entry }: { entry: UpdateEntry }) {
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl border border-accent-dim/50 bg-linear-to-br from-raised via-surface to-accent-dim/10 p-6 hover:border-accent-dim/80 transition-colors"
    >
...
function EntryCard({ entry, isLast }: { entry: UpdateEntry; isLast: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 min-w-0 -mt-1 ${isLast ? '' : 'pb-9'}`}>
        <div className="rounded-xl border border-line bg-surface p-5 hover:border-accent-dim/70 hover:bg-raised/30 transition-colors">
    ...
    <div className="p-10 pt-15 max-w-8xl mx-auto">
      <ScrollReveal>
        <div className="flex items-center gap-3.5 mb-10">
          <span className="w-11 h-11 rounded-xl bg-accent/10 border border-accent-dim/30 flex items-center justify-center">
            <IconBell className="w-5 h-5 text-accent-bright" />
          </span>
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted mt-0.5 flex items-center gap-1.5 flex-wrap">
              ...
              {fromCache && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-raised border border-line text-[10px] text-muted/70">
          ...
          <div className="ml-auto shrink-0">
            <button
              ...
              title={tc('refresh')}
              className="focus-ring cursor-pointer p-2 rounded-lg text-muted/60 hover:text-ink hover:bg-raised transition-colors"
            >
      ...
      {loading && entries.length === 0 ? (
        <div className="flex flex-col gap-4">
          {Array.from({ length: 2 }).map((_, i) => (
            <div
              key={i}
              className="h-32 rounded-xl border border-line bg-surface animate-pulse"
            />
          ))}
        </div>
      ) : entries.length === 0 ? (
