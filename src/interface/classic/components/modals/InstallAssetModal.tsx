      ...
      >
        <div className="flex items-start gap-3 p-5 pb-4">
          ...
          <button
            onClick={onClose}
            disabled={installing}
            className="focus-ring cursor-pointer p-1.5 rounded-lg text-muted/50 hover:text-ink hover:bg-raised transition-colors disabled:opacity-40"
            aria-label={t('close')}
          >
        ...
        <div className="px-5 pb-3">
          <div className="flex items-center gap-1 p-1 rounded-xl border border-line bg-raised">
            <button
              ...
              className={`focus-ring cursor-pointer flex-1 flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                tab === 'project'
        ...
        <div className="px-5 flex flex-col gap-3 min-h-0 flex-1">
          {canCreateTemplate && tab === 'template' && (
            <button
              ...
              disabled={installing}
              className="focus-ring cursor-pointer w-full flex items-center gap-3 p-3 rounded-xl border border-dashed border-mint/30 bg-mint/5 hover:bg-mint/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-left"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-lg border border-line bg-surface text-sm text-ink placeholder:text-muted/50 outline-none transition-all focus:border-accent focus:ring-1 focus:ring-accent/30"
            />
          </div>

          <div className="flex-1 min-h-0 max-h-72 overflow-y-auto flex flex-col gap-1.5 pb-1">
            {rows.length === 0 ? (
              <div className="border border-dashed border-line rounded-xl py-10 flex flex-col items-center gap-2 text-center">
              ...
              rows.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`focus-ring cursor-pointer w-full flex items-center gap-3 p-2.5 rounded-xl border text-left transition-colors ${
                      selected
                        ? 'border-accent-dim/60 bg-accent/10'
                        : 'border-transparent hover:bg-raised hover:border-line'
                    }`}
                  >
                    <span
        ...
        {error && (
          <div className="mx-5 mt-3 flex items-start gap-2 p-2.5 rounded-lg bg-danger/10 border border-danger/20 text-xs text-danger leading-snug">
        ...
        <div className="flex items-center justify-end gap-2.5 p-5 pt-4 border-t border-line mt-3">
          <button
            onClick={onClose}
            disabled={installing}
            className="focus-ring cursor-pointer px-4 py-2.5 rounded-lg text-sm text-muted hover:text-ink hover:bg-raised transition-colors disabled:opacity-40"
          >
            {t('cancel')}
          </button>
          <motion.button
            ...
            className="focus-ring cursor-pointer flex items-center gap-2 px-4 py-2.5 rounded-lg bg-accent/15 border border-accent-dim/40 text-accent-bright text-sm font-medium transition-colors hover:bg-accent/25 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {installing ? (
              <>
